...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
        GameObject.Find("Level").GetComponent<Text>().text = "Level " + Maze;
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
                ...
                // holes in the floor have a 2% chance of spawning (but not in the player's starting position)
                if (Random.Range(1, 50) != 1 | x == start_x & z == start_z) {
