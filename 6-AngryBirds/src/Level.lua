...
function Level:init()
    ...
    -- flag stores whether the player has collided with anything (to check for splitting)
    self.collided = false
    ...
    function beginContact(a, b, coll)
        ...
        -- if the player collides with anything, it can no longer split
    ...
    -- stores all active players in the scene (may be multiple due to splitting)
    self.players = {}

    -- flag stores whether the player has split yet
    self.split = false
...
function Level:update(dt)
    ...
    -- remove players if they have stopped moving
    ...
    -- replace launch marker if all players have stopped moving
    if self.launchMarker.launched then
        local xPos, yPos = self.launchMarker.alien.body:getPosition()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        -- if there are zero players (they've all stopped moving) respawn
        if xPos < 0 or xPos > VIRTUAL_WIDTH or (math.abs(xVel) + math.abs(yVel) < 1.5) and #self.players == 0 then
    ...
    -- allows player to split if the player has been launched but has not yet collided or split
    if self.launchMarker.launched and not self.collided and not self.split then
        if love.keyboard.wasPressed('space') then

            -- reference to the original player (middle)
            local originalPlayer = self.launchMarker.alien.body
            local x = originalPlayer:getX()
            local y = originalPlayer:getY()

            -- create a player 40px above the original
            local player2 = Alien(self.world, 'round', x, y - 40, 'Player')

            -- create a player 40px below the original
            local player3 = Alien(self.world, 'round', x, y + 40, 'Player')

            -- add velocities to the new players
            local originalVelocityX, originalVelocityY = originalPlayer:getLinearVelocity()

            player2.body:setLinearVelocity(originalVelocityX, originalVelocityY - 30)
            player3.body:setLinearVelocity(originalVelocityX, originalVelocityY + 30)

            -- add restitution and angular damping to both players
            player2.fixture:setRestitution(0.4)
            player3.fixture:setRestitution(0.4)
            
            player2.body:setAngularDamping(2)
            player3.body:setAngularDamping(2)

            table.insert(self.players, player2)
            table.insert(self.players, player3)

            self.split = true

            -- play a sound when the player splits
            gSounds['split']:play()
