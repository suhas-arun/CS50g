            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        -- get the increases in each stat
                        local HPInc, attackInc, defenseInc, speedInc = self.playerPokemon:levelUp()
                        
                        -- display level up

                        gStateStack:push(LevelUpMenuState({
                            -- the new stats for the pokemon
                        ...
                        function()
                            self:fadeOutWhite(true)
                        end))
                    else
                        self:fadeOutWhite(false)
                    end
                end)
            end)
        end))
...
-- boolean flag stores whether the pokemon just levelled and if true then there is another state to pop from the stack
function TakeTurnState:fadeOutWhite(levelledUp)
    -- fade in
    gStateStack:push(FadeInState({
        r = 255, g = 255, b = 255
    }, 1, 
    function()
        ...
        -- pop off the level up menu state
        if levelledUp then
            gStateStack:pop()
        end
