...
function Selection:init(def)
    ...
    -- allow cursor to be disabled (level up menu)
    self.cursorEnabled = def.cursorEnabled

    -- if cursorEnabled is not specified it defaults to true
    if self.cursorEnabled == nil then
...
function Selection:render()
    ...
    for i = 1, #self.items do
        ...
        -- draw selection marker if we're at the right index and the cursor is enabled
