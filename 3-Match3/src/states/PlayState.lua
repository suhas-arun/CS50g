...
function PlayState:swapTiles(tile1, tile2, swapBack)
    -- swapBack determines whether the tiles are swapped back if there is no match
...
function PlayState:calculateMatches(matches)
    ...
    local matches = matches or self.board:calculateMatches()
    if matches then
        gSounds['match']:stop()
        gSounds['match']:play()
        ...
        for k, match in pairs(matches) do
            ...
            self.timer = self.timer + #match -- Increments the timer by the number of matched tiles
        ...
        if not self.board:checkBoardMatches() then
            self.noMatches = true
            gSounds['error']:play()
            ...
            :finish(function()
                Timer.after(2, function()
                    ...
                    :finish(function()
                        while not self.board:checkBoardMatches() do
